...
if to == 'user' or service or is_admin1(msg) and to == "chat" or to == "channel" then
	if is_gbanned(msg.from.id) then
        return '<i>✨شما گولبال بن هستید✨.\n ✨برای ازاد کردن خود به @white_wolf_bot مراجعه کنیدد✨</i>'
	end
	...
	if matches[2]:lower() == 'english' and matches[3]:lower() == 'support' then
		...
		if is_banned(msg.from.id, tostring(target)) then
			return '<i>✨شما بن هستید.برای ازاد کردن خود به ربات @white_wolf_bot مراجعه کنید.✨</i>'
		end
	...
	elseif matches[2]:lower() == 'persian' and matches[3]:lower() == 'support' then
		...
		local long_id = data[tostring(target)]['long_id']
		if is_banned(msg.from.id, tostring(target)) then
			return '<i>✨شما بن هستید.برای ازاد کردن خود به SmartTG.ir یا ربات @white_wolf_bit مراجعه کنید✨</i>'
		end
	...
	if matches[1] == 'help' and msg.to.type == 'user' or matches[1] == 'pmhelp' and is_admin1(msg) and msg.to.type ~= 'user' then
		...
		text = " <i>✨به وای ولف خوش آمدید✨!\n\n>این ربات به هیچ وج رایگان نیست!\n\n\n\n\nبرای خرید به کانال ما مراجع فرمایید\n\n\n\n ch= @white_wolf_ch✨ </i>"
     	return text
    end
