...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "<i>✨خطا/چنین نام کاربری پیدا نشد</i>✨")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "<i>✨شما نمیتوانید مقامات بالا را اخراج کنید</i>✨")
			return
         end
      ...
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, '><i>✨ ['..user_id..' ] Banned for all  Groups/SuperGroups! (Globally banned)</i>✨')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, '><i>✨ ['..user_id..' ] Unbanned for all  Groups/SuperGroups! (Unglobally banned)</i>✨')
...
local support_id = msg.from.id
 if matches[1]:lower() == 'ایدی' and msg.to.type == "chat" or msg.to.type == "user" then
    if msg.to.type == "user" then
      return reply_msg(msg.id, "<i>✨ایدی ربات > |"..msg.to.id.."|\n✨ایدی شما > |"..msg.from.id.."| </i>", ok_cb, false)	
    end
  ...
  if matches[1]:lower() == 'بن' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "<i>✨شما نمیتوانید مقامات بالاتر را حذف کنید</i>✨"
        end
...
if matches[1]:lower() == 'اخراج' then
	...
	elseif string.match(matches[2], '^%d+$') then
		...
		if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
			return "<i>✨شمانمیتوانید مقامات بالاتر را اخراج کنید</i>✨"
		end
		if tonumber(matches[2]) == tonumber(msg.from.id) then
			return "!!"
		end
  ...
  if matches[1]:lower() == 'بن ال' and is_admin1(msg) then -- Global ban
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return '><i>✨ ['..user_id..' ] Banned for all  Groups/SuperGroups! (Globally banned)</i>✨'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return '><i>✨ ['..user_id..' ] Unbanned for all  Groups/SuperGroups! (Unglobally banned)</i>✨'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "گولبال بن لیست" then -- Global ban list
    return banall_list()
  end
end
