  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "<i>✨کاربر ["..msg.from.id.."]بخاطر اسپم بلاک شد(وایت ولف)✨</i>")
			savelog(msg.from.id.." PM", "<i>✨کاربر ["..msg.from.id.."] بخاطر اسپم بلاک شد(وایت ولف 8)✨</i>")
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "<i>✨اسپم اینجا مجازنیست✨\n@"..username.."["..msg.from.id.."]\n✨وضعیت:کاربرحذف شد(وایت ولف 8)✨</i>")
		else
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
		  ...
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "<i>✨کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." ازگروه ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] گولبال بن شد(spamming)✨</i>"
