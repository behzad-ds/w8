local function set_bot_photo(msg, success, result)
  ...
  if success then
    ...
    send_large_msg(receiver, '✨خطا, بعدا امتحان کنید!✨', ok_cb, false)
  end
end
...
local function run(msg,matches)
    ...
    if matches[1] == "setbotphoto" then
    	...
    	return '✨عکس جدید ژوپیتر ثبت شد✨'
    end
    if matches[1] == "markread" then
    	if matches[2] == "on" then
    		redis:set("bot:markread", "on")
    		return "✨خواندن پیام روشن شد✨"
    	end
    	if matches[2] == "off" then
    		redis:del("bot:markread")
    		return "✨خواندن پیام خاموش شد✨"
    	end
    	return
    end
    if matches[1] == "pm" then
    	local text = "✨پیغام از طرف✨ "..(msg.from.username or msg.from.last_name).."\n\n✨پیغام✨ : "..matches[3]
    ...
    if matches[1] == "pmblock" then
    	if is_admin2(matches[2]) then
    		return "✨شما نمیتوانید ادمین هارابلاک کنید✨"
    	end
    ...
    if matches[1] == "delcontact" then
      ...
      return "✨کاربر "..matches[2].." ازلیست مخاطبان ریمو شد✨"
    end
   ...
   return "✨کاربر باشماره تلفن +"..matches[2].." ذخیره شد✨"
end
 ...
 if matches[1] == "mycontact" and is_sudo(msg) then
	if not msg.from.phone then
		return "✨باید شماره تلفن شما در ربات ذخیره باشد✨"
    end
    ...
    if matches[1] == "sync_gbans" then
    	...
    	for k,v in pairs(jdat) do
			...
			banall_user(v)
      		print(k, v.." ✨گولبال بن شدند✨")
    	end
    end
	if matches[1] == 'reload' then
		receiver = get_receiver(msg)
		reload_plugins(true)
		post_msg(receiver, "✨در حال بروزرسانی✨", ok_cb, false)
		return "✨بروز شد✨"
	end
	--[[*For Debug*
	...
	if matches[1] == 'remlog' and not matches[2] then
		if not is_log_group(msg) then
			return "✨لاگ فعال نیست✨"
		end
		print("✨لاگ سوپر گروه "..msg.to.title.."("..msg.to.id..") حذف شد✨")
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] added Log_SuperGroup")
		logrem(msg)
	end
    return
